...
function tpl_mediaFileList() {
    ...
    // Hiển thị chọn để chuyển giữa các tab
    ...
    // Quản lý chức năng của từng tab
    echo '<div class="panelContent">'.NL;
    if($opened_tab == 'files') {
        media_tab_files($NS, $AUTH, $JUMPTO);       // Tab hiển thị file đã upload
    } elseif($opened_tab == 'upload') {
        media_tab_upload($NS, $AUTH, $JUMPTO);      // Tab upload file
    } elseif($opened_tab == 'search') {
        media_tab_search($NS, $AUTH);               // Tab search file
    }
    echo '</div>'.NL;
}
...
function tpl_media() {
    ...
    media_nstree($NS);                                  // Hiển thị các namespace
    echo '</div>'.NL;
    echo '</div>'.NL;

    echo '<div class="panel filelist">'.NL;
    tpl_mediaFileList();                                // Hiển thị phần file, upload, search
    echo '</div>'.NL;

    echo '<div class="panel file">'.NL;
    echo '<h2 class="a11y">'.$lang['media_file'].'</h2>'.NL;
    tpl_mediaFileDetails($image, $rev);                 // Hiện thị thông tin của file đã upload
    echo '</div>'.NL;
