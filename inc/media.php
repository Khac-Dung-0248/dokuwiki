...
function media_delete($id,$auth){
    ...
    // delete file
    $url = "http://localhost:3000/file-service/remove";
    $headers = array(
        "Accept: application/json, text/plain, */*",
        'Content-Type: application/json',
        "Cookie: access-token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..Gc2VbVQ9snEFiDwMi6GxgL0Zv1h-efRP_1gkZySITSk; refresh-token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2MjA0YzA2YTU2OGU1MDBhNzRmOTFhYTgiLCJpdiI6eyJ0eXBlIjoiQnVmZmVyIiwiZGF0YSI6Wzc2LDgyLDE5Myw5MywyMCw3NiwxOTQsMTA1LDc3LDY1LDE5LDIxMywxMTQsMjExLDk1LDIyMF19LCJ0aW1lIjoxNjQ1Njg2MTI2MjM4LCJpYXQiOjE2NDU2ODYxMjYsImV4cCI6MTY0ODI3ODE4Nn0.n0KUZSvrXKLQXAlfuSoqcOnhECjfnpywyGA3NMoSecI"
    );
    $data_delete_myDrive = array();
	...
	$json_array_data = json_decode($jsonContent, true);
    if(key_exists($id, $json_array_data)){
		$data_delete_myDrive[] = array('id' => $json_array_data[$id]);
		unset($json_array_data[$id]);
		$json = json_encode($json_array_data);
		file_put_contents("../../file.json", $json);
	}
    $data_delete = json_encode($data_delete_myDrive);

	curl_setopt($curl, CURLOPT_POSTFIELDS, substr($data_delete,1,33));
...
function media_save($file, $id, $ow, $auth, $move) {
    ...
    // post file to myDrive
    $jsonContent = file_get_contents(__DIR__."../../file.json");
	$json_array_data = json_decode($jsonContent, true);
    if(!key_exists($data[2],$json_array_data)){
        $url = "http://localhost:3000/file-service/upload";
        $headers = array(
            "Accept: application/json, text/plain, */*",
            "Content-Type: multipart/form-data",
            "Cookie: access-token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..Gc2VbVQ9snEFiDwMi6GxgL0Zv1h-efRP_1gkZySITSk; refresh-token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2MjA0YzA2YTU2OGU1MDBhNzRmOTFhYTgiLCJpdiI6eyJ0eXBlIjoiQnVmZmVyIiwiZGF0YSI6Wzc2LDgyLDE5Myw5MywyMCw3NiwxOTQsMTA1LDc3LDY1LDE5LDIxMywxMTQsMjExLDk1LDIyMF19LCJ0aW1lIjoxNjQ1Njg2MTI2MjM4LCJpYXQiOjE2NDU2ODYxMjYsImV4cCI6MTY0ODI3ODE4Nn0.n0KUZSvrXKLQXAlfuSoqcOnhECjfnpywyGA3NMoSecI"
        );
        $data_upload_myDrive = array(
            "filename" =>$data[1],
            "parent" => "/",
            "parentList" => "/",
            "currentID" => $data[2],
            "file"=> new CURLFILE($data[0],$data[3],$data[1])
        );
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_SSL_VERIFYPEER => false,
            CURLOPT_HTTPHEADER => $headers,
            CURLOPT_POSTFIELDS => $data_upload_myDrive
        ));
    
        //for debug only!
        $response_myDrive = curl_exec($curl);
        // curl_exec($curl);
        curl_close($curl);
        msg($response_myDrive);
    
        // Lưu id dokuwiki và id myDrive
        $json_array_data[$data[2]] = substr($response_myDrive,8,24);
        $json = json_encode($json_array_data);
        file_put_contents("../../file.json", $json);
    }
...
function media_filelist($ns,$auth=null,$jump='',$fullscreenview=false,$sort=false){
    ...
    if($auth < AUTH_READ){
        ...
        // $url = "http://localhost:3000/file-service/quick-list";

	    // $headers = array(
		//     "Accept: application/json, text/plain, */*",
		//     "Cookie: access-token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..fwR5SBoBDa18RhXGLfKlvY7zPTJQRD9-wrG5yGWSxSU; refresh-token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2MjA0YzA2YTU2OGU1MDBhNzRmOTFhYTgiLCJpdiI6eyJ0eXBlIjoiQnVmZmVyIiwiZGF0YSI6WzE2MSwyMTcsMjMxLDE2OCwzMSw1MCwxMDUsMjE1LDE1NSwyNSwyMSw4NiwyMTcsMjI4LDIyMCwxMTNdfSwidGltZSI6MTY0NTcxNTQ5NjE4MywiaWF0IjoxNjQ1NzE1NDk2LCJleHAiOjE2NDgzMDc1NTZ9._yZMwjb7rDRbNJJXsbiUl334MCDHNNBQbpKmRgBWBBM"
	    // );
	    ...
	    // //for debug only!
	    // $showMyDriveData = curl_exec($curl);
	    // curl_close($curl);
        // $data = json_decode($showMyDriveData);
        if(!count($data)){
            echo '<div class="nothing">'.$lang['nothingfound'].'</div>'.NL;
        }else {
            if ($fullscreenview) {
...
function media_tab_files($ns,$auth=null,$jump='') {
    global $lang;
    if(is_null($auth)) $auth = auth_quickaclcheck("$ns:*"); // Kiểm tra người dùng có quyền xem nội dung upload hay không

    if($auth < AUTH_READ){
        echo '<div class="nothing">'.$lang['media_perm_read'].'</div>'.NL;  // Nếu không có quyền xem
    }else{
        media_filelist($ns,$auth,$jump,true,_media_get_sort_type());        // Nếu có quyền xem và hiển thị list file đã upload
    }
}
...
function media_tab_upload($ns,$auth=null,$jump='') {
    global $lang;
    if(is_null($auth)) $auth = auth_quickaclcheck("$ns:*"); // Kiểm tra người dùng có quyền upload hay không
...
function media_uploadform($ns, $auth, $fullscreen = false) {
    ...
    $auth_ow = (($conf['mediarevisions']) ? AUTH_UPLOAD : AUTH_DELETE);         // Kiểm tra upload hay là xóa file
    ...
    $form->addTagOpen('p');
    $form->addTextInput('upload', $lang['txt_upload'])->id('upload__file')  // Input thực thi hành động select file upload
            ->attrs(['type' => 'file']);    
    $form->addTagClose('p');
    $form->addTagOpen('p');
    $form->addTextInput('mediaid', $lang['txt_filename'])->id('upload__name')   // Input hiển thị tên file muốn đặt sau khi chọn file
            ->val(noNS($id));
    $form->addButton('', $lang['btn_upload'])->attr('type', 'submit');    // Button submit upload (Action upload thì không phải ơ đây)
    $form->addTagClose('p');
    ...
    // Thông báo về kích thước tối đa của file
